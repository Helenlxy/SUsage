...
<div class="well col-md-8 col-md-offset-2 col-sm-10 col-sm-offset-1 text-center col-xs-10 col-xs-offset-1">
  <h3 style="color:#4CAF50">新 增 用 户</h3><br>
  ...
  <div class="input-group">
    <input class="form-control" placeholder="用户真实姓名" id="TrueName" onkeyup="if(event.keyCode==13)$('#SUsageID')[0].focus();">
  </div><br>
  
  <div class="input-group">
    <input class="form-control" placeholder="SUsage ID > 用户登录名" id="SUsageID" onkeyup="if(event.keyCode==13)$('#Dep')[0].focus();">
  </div><br>
  ...
  <button class="btn btn-info" style="width:100%" onclick="AddUser();" id="addbtn">新 增</button>
  <br>
</div>
</div>
</div>
...
function AddUser(){
 ...
 $("#SUsageID")[0].disabled=1;
 $("#Dep")[0].disabled=1;
 $("#addbtn")[0].disabled=1;
 var Truename = $("#TrueName").val();
 var SUsageID = $("#SUsageID").val();
 var Dep = $("#Dep").val();
 $("#addbtn").html("正在新增，请稍后……");
 $("#addbtn").removeClass("btn-info");
 $("#addbtn").addClass("btn-success");
 
 if(Truename == "" || SUsageID == ""){
  //未填写姓名或ID
  ...
  $.ajax({
   url:"DPtoAddUsr.php",
   type:"post",
   data:{tname:Truename,usr:SUsageID,dep:Dep},
